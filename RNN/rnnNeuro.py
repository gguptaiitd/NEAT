...
class RNN(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.bidirectional is True:
            h0 = Variable(torch.zeros(self.num_layers, batch_size, self.hidden_size).float())
        else:
            h0 = Variable(torch.zeros(self.num_layers*2, batch_size, self.hidden_size).float())
...
class LSTM(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.bidirectional is False:
            h0 = Variable(torch.zeros(self.num_layers*2, batch_size, self.hidden_size).float())
            c0 = Variable(torch.zeros(self.num_layers*2, batch_size, self.hidden_size).float())
        else:
...
class CustomModel():
    ...
    def test(self, test_loader, CUDA=True):
        """Evaluate a model."""
        self.model.eval()
        test_loss = 0
        correct = 0
